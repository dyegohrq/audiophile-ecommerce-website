...
export default function Category() {
  ...
  const products = [...categoryData].sort(
  ...
  return (
    <main>
      <h1
        className={`${style["text-present-4"]} bg-black pt-[140px] pb-[50px] text-white text-center `}
      >
        {" "}
        {name}{" "}
      </h1>
      <Container>
        {products.map((product, index) => (
          <section
            key={product.id}
            className={
              "py-[60px] w-full lg:flex flex-col items-center gap-[32px] lg:flex-row "
            }
          >
            ...
            >
              ...
              <Button
                classButton="orange"
                url={`/product/${product.category}/${product.id}/${product.slug} `}
              />
            </div>
          </section>
        ))}
